...
# MÍNIMOS QUADRADOS ORDINÁRIOS (MQO ou OLS)
...
$$\frac{\delta S}{\delta \alpha} = 0 \rightarrow \hat{\alpha} = \bar{y} - \beta \bar{x}$$
--

$$\frac{\delta S}{\delta \beta} = 0 \rightarrow \hat{\beta} = \frac{\sum x_t y_t - T \bar{x}\bar{y}}{\sum{x_t^2 - T\bar{x}^2}}$$
--
...
aux_lm <- lm(aux ~ X)
s <- summary(aux_lm)
bp_stat <- length(aux)*s$r.squared
